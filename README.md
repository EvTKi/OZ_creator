...
* `build_structure(self) -> List[Dict[str, Any]]`: Основной метод. Читает оба листа Excel (используя имена из конфигурации), обрабатывает данные и строит внутреннюю структуру категорий.
...
**Назначение:** Централизованное управление логированием приложения. Настраивает логирование как в файл (в папку `./log`), так и в консоль (или GUI).
...
1. **Инициализация (`main.py` или `gui.py`)**:
    ...
    - Создается экземпляр `LogManager`, который настраивает логирование в файл и консоль (или GUI).
...
2. **Парсинг Excel (`ExcelParser`)**:
    ...
    - `build_structure()` вызывает `_read_table_by_header_df()` для каждого из двух листов Excel (используя имена из конфигурации).
...
4. **Завершение (`main.py` или `gui.py`)**:
